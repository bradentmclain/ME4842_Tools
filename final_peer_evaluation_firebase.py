...
if st.user.is_logged_in:
	...
	if is_allowed(sign_in_id):
		...
		####Begin UI Here
		st.title("ME4842 Final Peer Evaluation")
		...
		ref = db.reference(f"Final_Peer_Evaluations/{st.session_state['active_user']}")
		...
		st.header("Final Peer Evaluation")
		if st.session_state['active_section'] != 'Click to Select':
			...
			if group != 'Click to Select':
				...
				for student in students:
					...
					st.markdown(f"##### Evaluation of {'your' if student == st.session_state.active_user else student+"'s"} participation during the three standard labs and memos.")
					labs_and_memos = st.number_input(f"How much this person contributed to the three standard lab experiments and memos on a scale from 0 to 5. 0 is no contribution, 5 is full expected individual contribution.", key=f"labs_{student}", min_value=0.0, max_value=5.0, step=0.1, format="%.1f", value=None)
					...
					final_project = st.number_input(f"How much this person contributed to the Final Experiment materials on a scale from 0 to 5. 0 is no contribution, 5 is full expected individual contribution.", key=f"final_{student}", min_value=0.0, max_value=5.0, step=0.1, format="%.1f", value=None)
				...
				st.markdown(f"##### OPTIONAL: List any overall ME4842 feedback that you have."); 
				me4842_comments = st.text_area(f"Examples: 1.) Did you appreciate being able to choose two out of the three standard labs you conducted? 2.) Keep the initial concept generation? Keep the initial concept generation revision? 3.) How can we better explain what is needed to make a good final experiment? 4.) Any suggestions for improvement of the class.")
				...
				if st.button("Submit Feedback", disabled = not is_complete):
					if not st.session_state.awaiting_confirm:
						if ref.get():
							...
							if me4842_comments != '':
								ref.child("me4842_comments").push(me4842_comments)
							show_review_prompt()
